...
def _build_mcr(cmd_type: int, sub: int, status: int, func_code: int,
               side: int, payload: bytes = b"") -> bytes:
    """Build an MCR frame."""
    header = bytes([
        cmd_type, 0x00, 0x00,
        (sub >> 8) & 0xFF, sub & 0xFF,
...
class SleepNumberBed:
    ...
    async def _send(self, client: BleakClient, data: bytes, timeout: float = 10.0) -> list[bytes]:
    ...
    async def _init_handshake(self, client: BleakClient) -> bool:
        ...
        result = await self._send(
            client,
            _build_mcr(MCR_CMD_PUMP, 0x0000, MCR_STATUS_PUMP, MCR_FUNC_INIT, 0, b"\x00" * 8),
            timeout=10.0,
        )
        return len(result) > 0
    ...
    async def _connect_and_init(self, device: BLEDevice) -> BleakClient | None:
        ...
        client = await establish_connection(BleakClient, device, self._address, max_attempts=3)
    ...
    async def async_connect_and_read(self, device: BLEDevice) -> BedStatus | None:
        ...
        try:
            ...
            try:
                ...
                result = await self._send(
                    client,
                    _build_mcr(MCR_CMD_PUMP, self._bed_addr, MCR_STATUS_PUMP,
                               MCR_FUNC_READ, 0x0F),
                )
                pump = None
                for data in result:
                ...
                if pump:
                    ...
                    _LOGGER.debug("Pump: L=%s R=%s", status.left_sleep_number, status.right_sleep_number)
                else:
                    _LOGGER.warning("Failed to parse pump status")
                    return None

                # Read bed presence per side (func=24)
                for side, side_name in [(SIDE_LEFT, "left"), (SIDE_RIGHT, "right")]:
                    result = await self._send(
                        client,
                        _build_mcr(MCR_CMD_PUMP, self._bed_addr, MCR_STATUS_PUMP,
                                   MCR_FUNC_PRESENCE, side),
                        timeout=5.0,
                    )
                    for data in result:
                        if (len(data) >= 13 and data[0] == 0x16 and data[1] == 0x16
                            ...
                            _LOGGER.debug("Presence %s: %s", side_name, present)
                            break

                # Read foundation positions (func=5, cmd=0x42, status=0x42)
                result = await self._send(
                    client,
                    _build_mcr(MCR_CMD_FOUNDATION, self._bed_addr,
                               MCR_STATUS_FOUNDATION,
                               MCR_FUNC_FOUNDATION_POSITIONS, 0x0F),
                    timeout=5.0,
                )
                ...
                if positions:
                    ...
                    _LOGGER.debug(
                        ...
                        status.left_head_position, status.left_foot_position,
                        status.right_head_position, status.right_foot_position,
                    )
                else:
    ...
    async def async_set_sleep_number(self, device: BLEDevice, side: int, value: int) -> bool:
        ...
        try:
            ...
            try:
                await self._send(
                    client,
                    _build_mcr(MCR_CMD_PUMP, self._bed_addr, MCR_STATUS_PUMP,
                               MCR_FUNC_SET, side, bytes([0x00, value])),
                    timeout=5.0,
                )
                return True
            finally:
    ...
    async def async_set_preset(self, device: BLEDevice, preset: int, side: int | None = None) -> bool:
            ...
            try:
                for s in sides:
                    await self._send(
                        client,
                        _build_mcr(MCR_CMD_FOUNDATION, self._bed_addr,
                                   MCR_STATUS_FOUNDATION, MCR_FUNC_PRESET,
                                   s, bytes([preset, 0x00])),
                        timeout=3.0,
                    )
                return True
        ...
        try:
            ...
            try:
                ...
                await self._send(
                    client,
                    _build_mcr(MCR_CMD_FOUNDATION, self._bed_addr,
                               MCR_STATUS_FOUNDATION, MCR_FUNC_OUTLET,
                               OUTLET_UNDERBED_LIGHT, bytes([mode, 0, 0])),
                    timeout=3.0,
                )
                return True
            finally:
        ...
        try:
            ...
            try:
                await self._send(
                    client,
                    _build_mcr(MCR_CMD_FOUNDATION, self._bed_addr,
                               MCR_STATUS_FOUNDATION, MCR_FUNC_SET,
                               side, bytes([head, 0, foot, 0])),
                    timeout=5.0,
                )
                return True
            finally:
